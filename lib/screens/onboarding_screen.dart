...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  void initState() {
    ...
    _currentMonth = profile.currentMonth;
    _specialSituations = profile.specialSituations;

    _ageController = TextEditingController(text: _age.toString());
    _incomeController = TextEditingController(text: _annualIncomeManwon.toString());
    _religiousDonationController = TextEditingController(
      text: ((_specialSituations.monthlyReligiousDonation ?? 0) / 10000).round().toString(),
    );
                      ...
                      children: <Widget>[
                        ...
                        Expanded(
                          child: AppButton(
                            ...
                            child: Text(_step == _totalSteps ? '상세 입력 시작하기' : '다음'),
                          ),
                        ),
                      ],
                    ),
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        LabeledInput(
          ...
          suffixText: '만원',
          helperText: '세전 연봉 기준으로 입력하세요.',
          onChanged: (value) => _annualIncomeManwon = int.tryParse(value) ?? 0,
        ),
        const SizedBox(height: 12),
        Text('부양가족 수', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        Row(
          children: <Widget>[
            _roundControlButton(
              icon: Icons.remove,
              onTap: () => setState(() => _dependents = (_dependents - 1).clamp(0, 99)),
            ),
            SizedBox(
              width: 72,
              child: Text(
        ...
        const SizedBox(height: 8),
        Text('본인 제외, 소득요건 충족 가족 수를 입력합니다.', style: Theme.of(context).textTheme.bodySmall),
        const SizedBox(height: 14),
        Text('현재 월', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        DropdownButtonFormField<int>(
          value: _currentMonth,
          items: List<DropdownMenuItem<int>>.generate(12, (int index) {
            final month = index + 1;
            return DropdownMenuItem<int>(value: month, child: Text('$month월'));
          }),
          onChanged: (value) {
            if (value != null) setState(() => _currentMonth = value);
          },
        ),
      ],
    );
      ...
      children: <Widget>[
        ...
        _checkboxTile(
          ...
          onChanged: (value) {
            setState(() {
              _specialSituations =
                  _specialSituations.copyWith(isSMEYouthTaxReduction: value);
            });
          },
        ),
        _checkboxTile(
        ...
        if (_specialSituations.hasRent) ...<Widget>[
          ...
          LabeledInput(
            ...
            onChanged: (value) {
              final manwon = double.tryParse(value) ?? 0;
              _specialSituations =
                  _specialSituations.copyWith(monthlyRent: manwon * 10000);
            },
          ),
        ],
        _checkboxTile(
          ...
          onChanged: (value) =>
              setState(() => _specialSituations = _specialSituations.copyWith(hasChildren: value)),
        ),
        _checkboxTile(
          label: '장애인 부양가족이 있음',
          ...
          onChanged: (value) => setState(
            () => _specialSituations =
                _specialSituations.copyWith(hasDisabledDependent: value),
          ),
        ),
        _checkboxTile(
          ...
          onChanged: (value) => setState(
            () =>
                _specialSituations = _specialSituations.copyWith(hasElderlyDependent: value),
          ),
        ),
      ],
    );
  ...
  Widget _buildStepResult() {
    final chips = <String>[
      if (_specialSituations.isSMEYouthTaxReduction && _age <= 34) '중소기업 청년 소득세 감면 (90%)',
      '신용/체크카드 소득공제',
      '연금저축 세액공제',
    ...
    return Column(
      ...
      children: <Widget>[
          ...
          child: Column(
            children: <Widget>[
              Container(
                ...
                child: const Icon(Icons.check, color: AppTheme.primary, size: 34),
              ),
              const SizedBox(height: 12),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(colors: <Color>[AppTheme.primary, Color(0xFF42A5F5)]),
  ...
  void _handleNext() {
    ...
    _annualIncomeManwon = int.tryParse(_incomeController.text) ?? _annualIncomeManwon;
    ...
    final updatedProfile = widget.appState.userProfile.copyWith(
      ...
      specialSituations: _specialSituations.copyWith(
        monthlyReligiousDonation: _specialSituations.hasReligiousDonation
            ? ((double.tryParse(_religiousDonationController.text) ?? 0) * 10000)
            : null,
        clearMonthlyReligiousDonation: !_specialSituations.hasReligiousDonation,
        monthlyRent: _specialSituations.hasRent
